...
%McKay’s work provides a survey of MC-methods [14].
...
Če želimo oceniti $V_\pi(s)$, vrednost nekega stanja $s$ po politiki $\pi$, imenujemo vsak pojav stanja $s$ v enem od epizod {\em obisk (angl. visit)} stanja $s$. {\em Every-visit MC metoda} oceni $V_\pi(s) kot povprečje donosov po vsakem obisku stanja $s$ v vsaki epizodi, {\em first-visit MC metoda} pa povpreči samo donose po prvem obisku stanja $s$ v vsaki epizodi. Oba algoritma konverkirata k pričakovani vrednosti $V_\pi(s)$, ko gre število obiskov stanja $s$ v neskončnost [SINGH AND SUTTON, 1996 REF].

% algoritem

Pomemben dejavnik v MC metodah je to, da so ocene vsakega stanja neodvisne. Ocena enega stanja ne gradi na oceni kateregakoli drugega stanja, kot velja pri DP metodah. Lastnost grajenja ocene vrednosti enega stanja na podlagi ocene vrednosti sledečih stanj imenujemo {\em zagon (angl. bootstrapping)}.

Rečeno v grobem, Monte Carlo metode počakajo dokler ni celoten donos stanja po obisku znan, preden ga uporabijo za oceno $V_\pi(s_t)$. Enostavno pravilo posodobitve vrednosti stanja na podlagi every-visit MC metode je
\begin{equation}
V(s_t) \gets V(s_t) + \alpha[R_t - V(s_t)],
\end{equation}
kjer je $R_t$ dejanski donos in $\alpha$ konstanta, ki določa ???.
...
%require no model and are fully incremental, but are more complex to analyze
...
￼
%5.2. MONTE CARLO ESTIMATION OF ACTION VALUES 113 Carlo methods is to estimate q∗. To achieve this, we first consider another policy evaluation problem.
% i guess i use a partial model! CHECK MODEL DESCRIPTION IN ELEMENTS OF RL IN INTRO!
\section{Rezultati}
% graphs, errors
